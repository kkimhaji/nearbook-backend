import { Injectable, NotFoundException, ConflictException, BadRequestException, ForbiddenException, forwardRef, Inject } from '@nestjs/common';
...
export class FriendService {
  ...
  async sendRequest(requesterId: string, dto: SendFriendRequestDto) {
    ...
    if (!receiver) throw new NotFoundException('존재하지 않는 사용자입니다.');
    if (receiver.id === requesterId) throw new BadRequestException('자기 자신에게 친구 요청을 보낼 수 없습니다.');
    ...
    // WebSocket 알림
    this.gateway.emitFriendRequestReceived(receiver.id, {
  ...
  async acceptRequest(userId: string, friendshipId: number) {
    ...
    if (!friendship) throw new NotFoundException('친구 요청을 찾을 수 없습니다.');
    if (friendship.receiverId !== userId) throw new ForbiddenException('수락 권한이 없습니다.');
    if (friendship.status !== 'pending') throw new BadRequestException('처리할 수 없는 요청입니다.');
    ...
    // WebSocket 알림
    this.gateway.emitFriendRequestAccepted(friendship.requesterId, {
  ...
  async rejectRequest(userId: string, friendshipId: number) {
    ...
    if (!friendship) throw new NotFoundException('친구 요청을 찾을 수 없습니다.');
    if (friendship.receiverId !== userId) throw new ForbiddenException('거절 권한이 없습니다.');
    if (friendship.status !== 'pending') throw new BadRequestException('처리할 수 없는 요청입니다.');
