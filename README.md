...
실시간 이벤트 전달과 BLE 토큰 관리를 모두 Redis를 중심으로 설계했습니다.

BLE Device Token은 Redis TTL을 이용해 자동 만료되도록 구성했고, `@socket.io/redis-adapter`를 적용하여 여러 서버 인스턴스로 확장하더라도 동일한 방식으로 WebSocket 이벤트를 전달할 수 있도록 설계했습니다.

현재는 단일 서버 환경으로 운영하고 있지만, 서버 인스턴스가 늘어나더라도 추가적인 코드 변경 없이 동일한 구조를 유지할 수 있습니다.
